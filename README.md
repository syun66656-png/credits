...
- **알림 범위 주의**: 브릿지 서버가 1대이므로 인게임 알림은 그 서버 접속자에게만 간다. 유저가 4개 백엔드 어디에 있든 알림을 받게 하려면 Redis(`redis.enabled: true`)로 "지급 이벤트"를 전 서버에 브로드캐스트하는 확장이 필요하다(현재는 미구현 — 요청 시 추가).
