# 크레딧 (Credit)

비밀정원(마인팜 네트워크) 자체 **크레딧(캐시성 포인트)** 플러그인. Paper 26.2 / Java 25.

- **잔액의 단일 진실원은 MariaDB.** 모든 쓰기는 트랜잭션, 잔액은 절대 음수/중복지급 불가.
- **전 연산 비동기.** DB/HTTP/네트워크 I/O 는 전부 비동기 스레드에서 → TPS 영향 0. 비동기 스레드에서 Bukkit API 를 호출하지 않는다.
- **실패 시 안전(fail-safe).** MariaDB 연결 실패 시 크레딧 연산을 하지 않고 플러그인을 스스로 비활성화한다.

크레딧의 저장·조회·지급·차감은 전부 자체 구현 + MariaDB 다. [PlayerPoints](https://github.com/Rosewood-Development/PlayerPoints) 는 **참고용 소스**로만 사용했고 런타임 의존성이 아니다. (녹여낸 것: 저장소 추상화, 오프라인 UUID 기반 연산, 원자적 차감(잔액 부족 시 실패 반환), config 기반 메세지, 공개 API 설계. 우리 차이: MariaDB 단일, 금액 `long`/`BIGINT`, 공개 API 는 `CompletableFuture` 비동기, 홈페이지 결제 브릿지 통합.)

---

## 빌드

```bash
mvn clean package
```

- 산출물: `target/Credit.jar` (파일명은 한글 이슈 회피용 영어, 버전/스피곳 접미사 없음 — `pom.xml` 의 `finalName` 이 결정). 콘솔/플러그인 목록 표기는 `plugin.yml` 의 `name: 크레딧` 이라 한글로 나온다.
- **paperweight / reobf / remap 미사용.** Paper 는 26.1부터 서버 JAR 난독화를 폐기했으므로 Mojang 매핑을 그대로 쓴다. `paper-api` provided 의존성 하나면 충분하다.
- 런타임 라이브러리(HikariCP, MariaDB JDBC)는 shade 하지 않고 `plugin.yml` 의 `libraries:` 로 서버가 런타임에 Maven Central 에서 받는다. → jar 가 작고 클래스 충돌이 없다.

### 필요 툴체인
| 항목 | 값 |
|---|---|
| JDK | **25** (Maven 도 JDK 25 로 실행) |
| Paper API | `[26.2,26.3)` — 26.2 라인 최신 빌드 자동 해석 |
| 빌드 | Maven |

> 참고: 이 저장소의 CI/개발 샌드박스는 JDK 21 이고 `repo.papermc.io` 접근이 네트워크 정책으로 차단되어 있어 `mvn package` 를 그대로 실행할 수 없었다. 실제 배포 환경(JDK 25 + Paper 저장소 접근 가능)에서 빌드한다. Paper 좌표(`[26.2,26.3)`)는 빌드 시점의 26.2 최신 빌드에 맞춰 조정할 수 있다.

---

## 설정

`config.yml`

- `database.*` — MariaDB 접속 정보 + HikariCP 풀 크기.
- `homepage.base-url` / `homepage.plugin-key` / `homepage.poll-interval-seconds` — 홈페이지 결제 브릿지. `plugin-key` 는 절대 노출 금지(로그/채팅에 찍지 않는다).
- `display.suffix` / `display.thousands-separator` — 금액 표시(접미사, 3자리 콤마).
- `redis.*` — (선택) 교차서버 캐시 무효화. 끄면 캐시 없이 항상 DB 조회.

`messages.yml` — **플레이어에게 나가는 모든 문구를 여기서 수정**한다(하드코딩된 메세지 없음). 지원: MiniMessage 전체 태그(`<gold>`, `<gradient>`, `<hover>`, `<click>` …), 내부 플레이스홀더 `<amount>`(콤마)·`<suffix>`·`<player>`·`<uuid>`, **PlaceholderAPI `%...%`**(`%player_name%`, `%credit_balance%` 등 — PAPI 설치 시, 메세지를 보는 플레이어 기준으로 MiniMessage 파싱 전에 해석), 커스텀 글리프 `<glyph:이름>`.

> PAPI 확장이 메인 스레드를 가정할 수 있어, 명령어 응답은 DB 조회(비동기) 후 **메인 스레드에서 메세지 빌드·전송**한다(TPS 영향 없음). 자동충전 지급 알림(`charge-received`)도 메인 스레드 전송이라 `%...%` 가 안전하게 동작한다.

**Nexo 태그 (`<glyph:...>`, `<shift:...>`):** Nexo 가 설치돼 있으면 메세지를 **Nexo 의 MiniMessage 로 파싱**해 `<glyph:info>`·`<shift:-2>` 등 Nexo 태그가 그대로 렌더된다(우리 `<amount>`/`<suffix>`/`<player>` 도 함께 동작). Nexo API 를 리플렉션으로 호출하므로 컴파일 의존성이 없고, Nexo 미설치/버전 상이 시 자체 MiniMessage 로 안전하게 폴백한다. 단 Nexo 가 있으면 우리 자체 `glyphs:` 섹션(`<glyph>`)은 Nexo 글리프에 밀려 사용되지 않는다.

---

## 명령어 / 권한

| 명령어 | 권한 | 동작 |
|---|---|---|
| `/크레딧` | `credit.use`(기본 true, 플레이어 전용) | 본인 잔액 |
| `/크레딧 확인 <닉네임>` | `credit.admin` | 대상 잔액 |
| `/크레딧 지급 <닉네임> <금액>` | `credit.admin` | 지급(UPSERT) |
| `/크레딧 차감 <닉네임> <금액>` | `credit.admin` | 차감(원자적, 부족 시 거부) |
| `/크레딧 인증 <uuid>` | `credit.admin` | UUID → 마크 닉네임 |
| `/크레딧 리로드` | `credit.admin` | config.yml(표시 설정) + messages.yml 다시 읽기 |

관리자 서브명령어는 권한이 없으면 **탭완성/사용법에 노출되지 않으며**, 일반 유저가 서브명령어를 입력하면 "권한이 없습니다" 대신 **없는 명령어처럼**(`unknown-command` 문구) 처리된다 — 기능 존재를 노출하지 않는다. `unknown-command` 를 `""` 로 비우면 완전히 조용히(아무 문구 없이) 처리된다. (모든 메세지는 `""` 로 두면 전송이 생략된다.)

`/크레딧 리로드` 는 **표시 설정과 메세지만** 다시 읽는다(즉시 반영). DB/Redis/브릿지 폴링 등 연결 설정 변경은 안전을 위해 **서버 재시작**이 필요하다(런타임에 커넥션 풀/구독을 갈아끼우다 유실 나는 것보다 재시작이 무손실 원칙에 맞다). 닉네임↔UUID 변환은 온라인 → usercache → Mojang API(비동기) 순이며 오프라인 유저도 안전하다.

---

## 공개 API (캐시 상점 등이 사용)

Bukkit `ServicesManager` 에 `CreditAPI` 구현을 등록하고 정적 접근자 `CreditProvider.get()` 도 제공한다. 전부 비동기·원자적.

```java
CreditAPI credit = kr.scfarm.credit.api.CreditProvider.get();
if (credit != null) {
    credit.take(uuid, 1000, "SHOP_TAKE").thenAccept(ok -> {
        if (ok) { /* 결제 성공 */ } else { /* 잔액 부족 */ }
    });
}
```

메서드: `getBalance / give / take / set / pay` — 모두 `CompletableFuture`. `take` 는 잔액 부족 시 `false`(잔액 불변). 홈페이지 지급도 동일한 DAO 경로(잔액 UPSERT + 원장)를 타므로 지급 로직은 한 곳에만 존재한다.

---

## 데이터 (MariaDB)

- `credit_balance` — 잔액(단일 진실원).
- `credit_ledger` — append-only 원장(모든 증감 이력, 감사/복구용).
- `credit_processed_charge` — 홈페이지 결제 멱등성(charge_id PK 로 중복 지급 차단) + **분쟁 방지 감사 기록**: 자동충전 1건당 charge_id·UUID·닉네임·지급액·**지급 전 잔액·지급 후 잔액**·처리 시각을 지급 트랜잭션 안에서 확정해 한 행에 보존한다(지급과 기록이 원자적 — 반쪽 기록 불가). 같은 내용이 **3중**으로 남는다: ① DB(이 테이블) ② 서버 로그 `[크레딧 자동충전]` 라인 ③ **플러그인 폴더 yml** `plugins/Credit/charge-logs/charges-YYYY-MM.yml`(append-only, 사람이 바로 열람). yml 은 실제 지급 서버(브릿지)에서만 기록되며 `homepage.charge-log-file: false` 로 끌 수 있다.
- `credit_username_cache` — 닉네임 캐시(접속 시 갱신).

지급 = 원자적 UPSERT + 원장(같은 트랜잭션). 차감 = 조건부 `UPDATE ... WHERE balance >= ?`(영향 행 0 = 잔액 부족 → false). 시작 시 스키마 자동 생성.

---

## 홈페이지 결제 브릿지 (명세 8번)

- `poll-interval-seconds` 마다 비동기 폴링: `GET /api/plugin/charges/pending` (헤더 `x-plugin-key`).
- **지급 수량은 응답의 `credits` 필드다. `amount`(결제 금액, 원)를 그대로 지급하면 안 된다** — 환산 비율은 **1,000원 = 1크레딧**이므로 1,000배 과지급이 된다. `credits` 가 없는 구버전 응답만 `amount / 1000` 으로 폴백하고, 결과가 0 이하면 지급을 보류하고 에러 로그를 남긴다.
- 각 건을 **단일 MariaDB 트랜잭션**으로 "정확히 한 번" 지급:
  `INSERT processed_charge`(PK 충돌 = 이미 처리 → 재지급 안 함) + `UPSERT balance += credits` + `INSERT ledger(HOMEPAGE_CHARGE)`.
- 지급/스킵 무관하게 `POST /api/plugin/charges/complete {"id":...}` 로 보고(멱등). 보고 실패 시 다음 폴링에 재보고되어 수렴.
- **오프라인/미접속 UUID 도 지급된다**(UUID 기반 UPSERT — `Bukkit.getPlayer` 를 요구하지 않음). 네트워크 오류 → 다음 폴링 재시도, 401 → 키 오류 로그, DB 실패 → 보고 보류.
- **지급 알림:** 브릿지가 도는 서버에 접속 중인 유저에게만 지급 직후 인게임 알림(`charge-received`)을 보낸다. 오프라인/타 백엔드 접속자는 조용히 지급만 되고 별도 알림은 없다(잔액은 즉시 반영, `/크레딧` 으로 확인).

### 벨로시티 다중 백엔드 배치 (중요)

플러그인은 **크레딧을 쓰는 모든 백엔드**에 설치하되(명령어/API/placeholder 용), **브릿지 폴링은 네트워크 전체에서 한 서버에서만** 켠다:

- 한 서버만 `homepage.bridge-enabled: true`, 나머지는 `false`.
- 이유 1 — **감사 로그 집중**: `[크레딧 자동충전]` 로그 라인은 실제 지급(`PAID`)한 서버에만 남는다. 여러 서버가 폴링하면 멱등성(charge_id PK) 때문에 지급은 한 번뿐이지만 "어느 서버가 먼저 집었나"에 따라 로그가 랜덤하게 흩어진다. 한 서버로 고정하면 감사 로그가 항상 그 서버의 `logs/latest.log` 에 모인다.
- 이유 2 — **폴링 낭비 방지**: 4개 서버가 60초마다 각자 홈페이지 API 를 두드릴 필요가 없다.
- DB 기록(`credit_processed_charge`, `credit_ledger`)은 단일 MariaDB 라 어느 서버가 지급하든 **항상 중앙에 온전히** 남는다. 분쟁 조사는 이 DB 가 1차 근거이고, 로그 파일은 보조 교차검증용이다.
- **알림 범위(Redis 로 해결됨)**: `redis.enabled: true` 이면 지급 시 브릿지 서버가 알림 이벤트를 전 백엔드로 브로드캐스트하고, **유저가 접속한 바로 그 서버**가 인게임 알림을 전달한다. 브릿지 서버가 1대여도 유저가 main/sub1/sub2/minigame 어디에 있든 알림이 도달한다. Redis 를 끄면 브릿지 서버 접속자에게만 전달(폴백). 플러그인이 없는 서버(hub)는 구독자가 없어 자동 제외된다.

### 예시 배치 (hub / main / sub1 / sub2 / minigame)

- hub: **플러그인 미설치**(크레딧 미사용, 알림도 없음).
- main / sub1 / sub2 / minigame: 플러그인 설치 + `redis.enabled: true`(모두 같은 Redis).
- 이 중 **한 서버만** `homepage.bridge-enabled: true`(예: main), 나머지는 `false`.
- 결과: 폴링·지급·감사 로그는 main 에 집중되고, 지급 알림은 유저가 접속한 서버가 전달한다.
- 명세 7번(제재 동기화)은 이번 범위 밖. HTTP/헤더 구조만 재사용 가능하게 두었다.

---

## 콘솔 로드 메세지 (전 플러그인 통일 포맷)

`(흰색)크레딧 (회색)- (성공=주황/실패=빨강)상태문구` — 예: `크레딧 - MariaDB 연결 성공`. Adventure `ComponentLogger` 로 실제 색을 찍는다. 색상 상수는 `util/ConsoleLog` 에 모아 두어 다른 플러그인과 통일하기 쉽게 했다.

---

## PlayerPoints 소스·버그픽스 대조 반영 내역

PlayerPoints 레포(커밋 히스토리 포함)를 실제로 클론해 대조했다. 대중적으로 검증되며 고쳐진 버그들을 다음과 같이 흡수했다:

**흡수한 패턴/픽스**
- **DB 닉네임 캐시 테이블** (`credit_username_cache`) — PlayerPoints `_2_Add_Table_Username_Cache` 마이그레이션 + `NameFetcher` 조회 사슬 패턴. 접속 시 UPSERT, 조회 성공 시 역기입. 멀티 백엔드 네트워크에서 "다른 서버로만 접속했던" 유저도 Mojang 호출 없이 닉네임↔UUID 해석. 닉변으로 같은 이름이 여러 UUID 에 남는 경우 **최근 갱신 우선**으로 택해 PlayerPoints 의 임의-1건 선택보다 안전.
- **캐시 수명주기** (`pointsCache` 의 preload/expire/refresh 설계 + placeholder 지연 픽스 `1ee9c1c` + 큐 무한누적 픽스 `c031f8b` 교훈) — placeholder 캐시는 **온라인 유저만** 담고(퇴장 시 제거 → 무한 증가 불가), 접속 시 예열, `cache.refresh-seconds`(기본 30초) 주기의 배치 쿼리 1회로 일괄 갱신. Redis 없이도 교차서버 placeholder 스테일이 이 주기로 수렴. 홈페이지 브릿지 지급 직후에도 즉시 캐시 통지.
- **동시 변경 잠금** (`ffebabf` "Lock on point modifications to prevent duplications" 의 DB 레벨 대응) — `pay` 는 행 잠금을 항상 UUID 사전순으로 획득해 상호 이체 데드락을 원천 차단하고, 모든 쓰기 트랜잭션에 데드락(1213/40001)·락 타임아웃(1205) 감지 시 짧은 백오프 재시도를 넣었다(트랜잭션이 자기완결적이라 재실행 안전).
- **정확한 닉네임 매칭** (`740c39b` #83 — 부분일치로 엉뚱한 유저에게 지급되던 버그) — `getPlayerExact` + 정확 일치 DB 조회만 사용.
- **HikariCP keepalive** — 유휴 커넥션이 방화벽/wait_timeout 에 조용히 끊겨 첫 연산이 실패하는 사고 방지(5분 ping).

**우리가 이미 회피했거나 더 안전한 부분**
- take 가 잔액을 0 으로 클램프하던 버그(`4664376` 로 그들이 거부 방식으로 수정) → 우리는 처음부터 조건부 UPDATE 거부.
- 시작잔액 중복 지급(`b6921b0`) → 우리는 읽기 시 행을 만들지 않아 원천 면역.
- importlegacy SQL 인젝션(`e874f4b`) → 전부 PreparedStatement.
- 10틱 메모리 배칭 후 플러시(그들 구조, `639f48e` 로 계속 보강) → 크래시 시 유실 창이 존재하는 구조라 채택하지 않음. 우리는 연산마다 즉시 트랜잭션(무손실 우선, 명세 0번).
- `NameFetcher` 가 아직 쓰는 **폐기된 Mojang `/user/profiles/<uuid>/names` 엔드포인트**는 답습하지 않고 현행 sessionserver 를 사용. 네거티브 캐싱(레이트리밋 방지)은 동일하게 채택.

## 실사용 전 보안·무결성 감사 (5개 관점 심층 점검 결과 반영)

배포 전에 ①금전 안전/DB 정합성 ②동시성/스레드 안전 ③브릿지 멱등성·HTTP ④수명주기/설정/운영 ⑤명령어·API·메세지 의 5개 관점으로 전수 감사하고, 발견된 결함을 모두 수정했다.

**치명적(금전) — 수정 완료**
- `closeQuietly` 의 `setAutoCommit(true)` 제거: JDBC 규약상 이 호출은 **열린 트랜잭션을 커밋**하고 HikariCP 의 반납 시 자동 롤백까지 무력화한다. 게다가 `catch` 가 `SQLException` 만 잡아 `RuntimeException`/`Error` 는 롤백을 건너뛰었다 → 이체 도중 오류 시 **입금만 커밋되어 돈이 생성**(또는 출금만 커밋되어 소멸)될 수 있었다. 모든 트랜잭션 catch 를 `Throwable` 로 확대.
- `set()` 의 lost update: 잠금 없는 `SELECT` 후 덮어써서 그 사이 커밋된 충전이 통째로 사라지고, 원장에는 거짓 delta(0)가 남아 **유실을 추적조차 못 하는** 상태였다 → `SELECT ... FOR UPDATE`(+행 선생성으로 갭 락 데드락 회피).
- 커밋 후 캐시/알림 처리에서 예외가 나면 **커밋된 지급이 실패로 보고**되어 관리자·상점이 재시도 → 이중 지급. 후처리를 전부 예외 격리.
- `commit()` 실패는 재시도 금지(`CommitAmbiguousException`): 커밋이 서버에선 성공했는데 응답만 유실된 경우 재실행하면 두 번 반영된다.
- 금액 상한(1조) 도입: 오타/홈페이지 버그로 천문학적 금액이 지급되거나 BIGINT 오버플로로 계정이 영구 고장나는 것을 차단.

**보안 — 수정 완료**
- 브릿지 HTTP 를 `Redirect.NEVER` 로: JDK HttpClient 는 **교차 호스트 리다이렉트에도 `x-plugin-key` 를 재전송**한다(도메인 탈취/오픈 리다이렉트 → 비밀키 유출 + 위조 pending 응답으로 임의 크레딧 발행). 또한 301/302 는 POST 를 본문 없는 GET 으로 바꿔 완료 보고를 조용히 유실시킨다. 3xx 는 설정 오류로 ERROR.
- `plugin-key` 시작 시 문자 검증: 키에 한글/공백/보이지 않는 문자가 섞이면 요청 생성 예외 메세지에 **키 전문이 담겨 로그 파일에 남는다**. 네트워크 예외 로그도 메세지 대신 예외 타입만 기록.
- `base-url` https 강제(평문 전송 방지) + 형식 검증, 응답 본문 크기 상한(스트림 단위).
- 지급 로그 yml 의 제어문자 이스케이프: 닉네임에 개행을 넣어 **가짜 지급 항목을 위조**하거나 월 파일 전체를 파싱 불가로 만들 수 있었다.

**정확성/운영 — 수정 완료**
- 지급하지 않은 건에 완료 보고 금지(`REJECTED`): 기존에는 비정상 금액을 조용히 "처리완료"로 보고해 **유저는 결제했는데 아무것도 못 받고 기록도 남지 않았다**. 이제 미지급으로 남기고 ERROR(건당 1회) → 수동 정산.
- 홈페이지 `amount` 엄격 파싱(정수 JSON 만), 닉네임/`charge_id` 길이 검증 — 참고용 필드 하나가 결제를 영구히 막던 문제 제거.
- 스키마 초기화를 네임드 락으로 직렬화 + `information_schema` 기반 조건부 마이그레이션: 4개 백엔드 동시 재시작 시 DDL 경합으로 한 서버가 자기 비활성화되는 것을 방지(MySQL 호환성도 확보).
- JDBC `socketTimeout` 지정: 없으면 스키마 초기화(메인 스레드)가 무한 대기해 **서버가 영영 부팅되지 않을** 수 있었다.
- messages.yml 에 jar 기본값 적용: 업데이트 시 새 키가 없어 플레이어에게 `<missing message: charge-received>` 가 노출되던 문제(결제 알림 포함) 해결. 리로드는 엄격 파싱 — YAML 오타 시 기존 문구를 유지하고 실패를 보고한다.
- placeholder 캐시: 온라인 유저만 보관 + 중복 적재 차단 + 퇴장 경합 정리(무한 증가/스테일 방지). PlaceholderAPI 확장은 비활성화 시 해제(클래스로더 누수 방지).
- 닉네임 오지급 방지: Mojang 조회 URL 인코딩(`Notch#1` 이 `Notch` 로 해석되던 문제), 닉네임 형식 검증, 지급/차감 성공 메세지에 **실제 반영된 UUID 표시**.
- `bridge-enabled` 기본값을 `false` 로(명시적 opt-in) — 신규 설치 시 전 백엔드가 동시 폴링해 감사 로그가 흩어지는 것을 방지.

## 명세와 다르게/추가로 결정한 사항

- **지급/차감에 `<금액>` 필수 인자 추가** — 원문에는 생략돼 있었으나 논리상 필요.
- **PlaceholderAPI 는 동기 API** 라 DB 를 블로킹할 수 없어, `%credit_*%` 는 캐시값을 반환하고 백그라운드로 최신화한다. 정확한 잔액은 항상 `/크레딧`(DB) 로 조회한다.
- **Redis 는 기본 off.** 무손실/정확성이 우선이므로 확신 없으면 캐시보다 매번 DB 조회를 택했다. 켜려면 `plugin.yml` 의 `libraries` 에 lettuce-core 를 추가한다.
- 비정상 금액(≤0)의 홈페이지 결제 건은 지급하지 않되 무한 재폴링을 막기 위해 `processed` 에만 기록해 수렴시킨다.
