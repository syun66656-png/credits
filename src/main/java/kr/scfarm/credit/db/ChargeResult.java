...
public enum ChargeResult {
    ...
    /**
     * 지급을 시도했지만 <b>잔액 검증에 실패</b>해 전체 롤백했음.
     *
     * <p>지급 전 잔액을 실측하고, 지급 후 잔액을 다시 실측해 {@code 지급후 - 지급전 == 지급수량} 인지
     * 대조한다. 이 등식이 깨지면(트리거/외부 수정/컬럼 클램프 등) 지급이 온전하지 않다는 뜻이므로
     * 커밋하지 않고 되돌린다. 완료 보고도 하지 않아 홈페이지에는 미지급으로 남는다 — 잔액이 어긋난 채
     * "처리완료"로 확정되는 것보다 안전하다.
     */
    BALANCE_MISMATCH,
